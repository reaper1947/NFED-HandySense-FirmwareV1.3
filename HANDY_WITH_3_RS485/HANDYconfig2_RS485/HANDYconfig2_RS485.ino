...
#include "TCA9548A.h"
float temp_1, humidity_1, temp_2, humidity_2, temp_3, humidity_3;
TCA9548A I2CMux;
Adafruit_SHT4x sht41_1 = Adafruit_SHT4x(); // Create instances for two SHT41 sensors
...
ModbusMaster    light, temp_rs485, Soil1_rs485, Soil2_rs485, Soil3_rs485;
MCP23008 MCP    (0x24);
...
Adafruit_SHT4x sht41 = Adafruit_SHT4x(); /* ประกาศตัวแปรเรียกใช้ SHT41 */
BH1750 lightMeter;
Max44009 myLux(0x4A);
...
const unsigned long eventInterval = 5 * 1000;
...
float ma_temp[5], ma_hum[5], ma1_soil[5], ma2_soil[5], ma3_soil[5], ma_lux[5];
...
unsigned int time_close[4][7][3] = { { { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 } },
  { { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 } },
  { { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 } },
  { { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 2000, 2000, 2000 }, { 52000, 2000, 2000 } }
};
...
int get_curentTimer() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (getLocalTime(&timeinfo)) {
      ...
      //Serial.print("weekdayNow     : "); Serial.print(weekdayNow);
      //Serial.println();
      //Serial.print("cal_weekdayNow : "); Serial.print(cal_weekdayNow);
      //Serial.println();
...
void Get_sht41() {
  ...
  if (!sht41.begin()) {
    // temp_error = 1; // Set error flag if initialization fails
    MCP.digitalWrite(status_sht31_error, LOW);
  } else {
    sensors_event_t hum, tem;
    sht41.getEvent(&hum, &tem); 
...
void Get_light() {
  ...
  //    //Serial.println("luxMy  : " + String(luxMy));
  //  } else {
  //    lux_error = 1;
...
void Get_soil() {
   uint8_t result_soil1, result_soil2, result_soil3;
   result_soil1 = Soil1_rs485.readHoldingRegisters(0, 2);
   delay(10);
   result_soil2 = Soil2_rs485.readHoldingRegisters(0, 2);
   delay(10);
   result_soil3 = Soil3_rs485.readHoldingRegisters(0, 2);
   if (result_soil1 == Soil1_rs485.ku8MBSuccess) {
     ma1_soil[4] = ma1_soil[3];
     ma1_soil[3] = ma1_soil[2];
     ma1_soil[2] = ma1_soil[1];
     ma1_soil[1] = ma1_soil[0];

     ma2_soil[4] = ma2_soil[3];
     ma2_soil[3] = ma2_soil[2];
     ma2_soil[2] = ma2_soil[1];
     ma2_soil[1] = ma2_soil[0];

     ma3_soil[4] = ma3_soil[3];
     ma3_soil[3] = ma3_soil[2];
     ma3_soil[2] = ma3_soil[1];
     ma3_soil[1] = ma3_soil[0];    
     //อ่านตำแหน่ง 0000
    //  float soil1 = Soil1_rs485.getResponseBuffer(0) /10.0f;
    //  float soil2 = Soil2_rs485.getResponseBuffer(0) /10.0f;
    //  float soil3 = Soil3_rs485.getResponseBuffer(0) /10.0f;

     ma1_soil[0] = Soil1_rs485.getResponseBuffer(0) /10.0f;
     ma2_soil[0] = Soil2_rs485.getResponseBuffer(0) /10.0f;
     ma3_soil[0] = Soil3_rs485.getResponseBuffer(0) /10.0f;
     soil = (((ma1_soil[0] + ma1_soil[1] + ma1_soil[2] + ma1_soil[3] + ma1_soil[4])/5.0f) +((ma2_soil[0] + ma2_soil[1] + ma2_soil[2] + ma2_soil[3] + ma2_soil[4])/5.0f)+((ma3_soil[0] + ma3_soil[1] + ma3_soil[2] + ma3_soil[3] + ma3_soil[4])/5.0f))/3;
    //  soil = (soil1+soil2+soil3)/3;
     Serial.print("soil1= ");
     Serial.println(ma1_soil[0]);
     Serial.print("soil2= ");
     Serial.println(ma2_soil[0]);
     Serial.print("soil3= ");
     Serial.println(ma3_soil[0]);
     Serial.print("soilSum= ");
     Serial.println(soil);
     soil_error = 0;
     MCP.digitalWrite(status_soil_error, HIGH);
   } else {
     soil_error = 1;
     Serial.println("soil_error  : " + String(soil_error));
  ...
  // buffer_soil = map(volts0, 1700000, 900000, 0, 100); /*buffer_soil = (-72.92 * abs(volts0)) + 230.76;*/
...
void ModbusToSlave(int _mode, int percent) {
  if (_mode == 1) {
    holdingRegisters[0] = _mode;
    holdingRegisters[1] = percent;
    modbus.writeMultipleHoldingRegisters(1, 0, holdingRegisters, 2);
  } else if (_mode == 2) {
    holdingRegisters[0] = _mode;
    holdingRegisters[1] = temp*100;
    holdingRegisters[2] = Max_Temp[0];
    holdingRegisters[3] = Min_Temp[0];
...
void setup() {
  delay(500);
  Serial.begin(115200); /* Connect to HandySense Dashboard 115200 */
  Serial2.begin(4800);
  EEPROM.begin(4096);
  Wire.begin();
  Wire.setClock(10000);
  ...
  //light.begin(4, Serial2); /* Modbus slave ID 1 */
  Soil1_rs485.begin(1, Serial2);
  Soil2_rs485.begin(2, Serial2);
  Soil3_rs485.begin(3, Serial2);
  //temp_rs485.begin(3, Serial2);
  ...
  xTaskCreatePinnedToCore(TaskWifiStatus, "WifiStatus", 6000, NULL, 1, &WifiStatus, 1);
...
//3wireSHT
  ...
  sensors_event_t humidityEvent_1, tempEvent_1;
  sht41_1.getEvent(&humidityEvent_1, &tempEvent_1);
  temp_1 = tempEvent_1.temperature;
  humidity_1 = humidityEvent_1.relative_humidity;
  I2CMux.closeChannel(1);
  delay(1);
  
  I2CMux.openChannel(2);
  sensors_event_t humidityEvent_2, tempEvent_2;
  sht41_2.getEvent(&humidityEvent_2, &tempEvent_2);
  temp_2 = tempEvent_2.temperature;
  humidity_2 = humidityEvent_2.relative_humidity;
  I2CMux.closeChannel(2);
  delay(1);
  
  I2CMux.openChannel(3);
  sensors_event_t humidityEvent_3, tempEvent_3;
  sht41_3.getEvent(&humidityEvent_3, &tempEvent_3);
  temp_3 = tempEvent_3.temperature;
  humidity_3 = humidityEvent_3.relative_humidity;
  I2CMux.closeChannel(3);
  delay(1);

  temp = (tempEvent_1.temperature+tempEvent_2.temperature+tempEvent_3.temperature)/3;
  humidity = (humidityEvent_1.relative_humidity+humidityEvent_2.relative_humidity+humidityEvent_3.relative_humidity)/3;
...
void TaskWifiStatus(void* pvParameters) {
  while (1) {
    ...
    do {
      Serial.println("ssid = " + String(ssid.c_str()) + " pass = " + String(password.c_str()) + " Connecting... ");
      delay(12000);
